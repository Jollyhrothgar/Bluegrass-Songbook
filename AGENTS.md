...
### Regression Testing (REQUIRED for Parser Changes)

**ALWAYS use regression testing when modifying parser logic to catch unintended side effects.**
...
**What it does:**
1. Creates baseline analysis from current output (17K+ files)
2. Runs batch process with your changes
3. Creates post-change analysis
4. Compares before/after, flagging:
   - Files with >10% change in verse/chord/word counts
   - Improvements vs regressions
   - Files needing manual review

**Output:**
- `analysis_before_<name>/` - Baseline metrics
- `analysis_after_<name>/` - Post-change metrics
- Comparison report with change categories
- Summary statistics

**Quick rerun** (if batch already completed):
```bash
python3 scripts/regression_test.py --name <name> --skip-baseline --skip-batch
```

**Rollback if needed** (output/ is tracked in git):
```bash
# Rollback all changes
git checkout HEAD -- output/

# Rollback specific files
git checkout HEAD -- output/problem_file.pro

# Or revert parser code and regenerate
git checkout HEAD -- src/chordpro_parser/parser.py
uv run python3 batch_process.py
```

### Making Changes (Standard Workflow)

1. Edit `src/chordpro_parser/parser.py`
2. **Run regression test**: `python3 scripts/regression_test.py --name <change_name>`
3. Review comparison report for regressions
4. If clean, generate spot-check: `uv run python3 create_new_spot_check.py`
5. Start viewer: `python3 viewer/server.py`
6. Manually validate random samples at http://localhost:8000
7. Check `viewer/feedback.jsonl` for results
