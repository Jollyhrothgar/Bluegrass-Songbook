...
# Get the source root directory (parent of viewer directory)
SOURCE_ROOT = Path(__file__).parent.parent
VIEWER_DIR = Path(__file__).parent

# Change to source root to ensure relative paths work
...
class ValidatorServer(SimpleHTTPRequestHandler):
    ...
    def serve_file_list(self):
        ...
        try:
            ...
            # Try spot check sample first, then minimal, then full sample, fall back to random
            if (viewer_dir / 'stratified_sample_spot_check.json').exists():
                with open(viewer_dir / 'stratified_sample_spot_check.json', 'r') as f:
                    sample = json.load(f)
            ...
            elif (viewer_dir / 'stratified_sample_minimal.json').exists():
                with open(viewer_dir / 'stratified_sample_minimal.json', 'r') as f:
                    sample = json.load(f)
            ...
            elif (viewer_dir / 'stratified_sample.json').exists():
                with open(viewer_dir / 'stratified_sample.json', 'r') as f:
                    sample = json.load(f)
                ...
                # No sample file - return empty list with instructions
                files = []
    ...
    def serve_random_file(self):
        ...
        try:
            ...
            html_path = Path('raw') / html_filename
